...
/* ----------------------------- TOP BAR ----------------------------- */
const NAV_ITEMS = ['equipo', 'contextos', 'preguntas', 'acceso', 'reportes', 'ajustes'];
const navLabel = (id) => ({ equipo: window.L('Equipo', 'Team'), contextos: window.L('Vacantes & Productos', 'Vacancies & Products'), preguntas: window.L('Preguntas', 'Questions'), acceso: window.L('Acceso', 'Access'), reportes: window.L('Reportes', 'Reports'), ajustes: window.L('Ajustes', 'Settings') }[id]);
...
const TeamCard = ({ p, onOpen }) => {
  ...
  return (
    <div onClick={() => onOpen(p)}
      style={{
        padding: '20px', borderRadius: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)',
          ...
          {DIMENSIONS().map((d, i) => (
            <div key={d} style={{ display: 'grid', gridTemplateColumns: '140px 1fr 32px', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ height: 6, borderRadius: 3, background: 'rgba(255,255,255,0.06)' }}>
                <div style={{
                  width: `${person.skills[i]}%`, height: '100%', borderRadius: 3,
    ...
    <div className="s-stage"><div className="s-wrap" style={{ maxWidth: 1280 }}>
      ...
      <div className="glass" style={{ padding: 0 }}>
        ...
        {questions.length === 0 ? (
          ...
          <div key={q.id}
            style={{
              display: 'grid', gridTemplateColumns: '1fr 120px 72px 90px 80px 36px', gap: 16, alignItems: 'center',
  ...
  const typeBadge = (t) => {
    ...
    return (
      <span className="mono" style={{
        fontSize: 9, padding: '2px 7px', borderRadius: 5, letterSpacing: '0.14em',
    ...
    <div className="s-stage"><div className="s-wrap" style={{ maxWidth: 1280 }}>
        ...
        {reports.map(r => (
          <div key={r.key} className="glass" style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 0 }}>
            ...
            <button
              className="btn"
              style={{
                width: '100%', justifyContent: 'center', padding: '9px', display: 'inline-flex', alignItems: 'center', gap: 6,
      ...
      <div className="glass" style={{ padding: 0 }}>
        ...
        {history.length === 0 ? (
          ...
          <div key={i}
            style={{
              display: 'grid', gridTemplateColumns: '1fr 64px 120px 72px 100px', gap: 16, alignItems: 'center',
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        ...
        {savedAt && !dirty && (
          <div className="mono" style={{
            fontSize: 10, color: 'rgba(120,255,180,0.65)', letterSpacing: '0.14em', textTransform: 'uppercase',
        ...
        {dirty && (
          <div className="mono" style={{
            fontSize: 10, color: 'rgba(251,191,36,0.75)', letterSpacing: '0.14em', textTransform: 'uppercase',
            display: 'flex', alignItems: 'center', gap: 6
          }}>
  ...
  return (
    <div className="s-stage"><div className="s-wrap" style={{ maxWidth: 900 }}>
      ...
      <div className="glass" style={{ padding: '28px 32px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 24 }}>
        <div style={{
          width: 64, height: 64, borderRadius: 16, background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)',
      ...
      <div className="glass" style={{ padding: '20px 28px' }}>
        <div className="mono" style={{
          fontSize: 9.5, letterSpacing: '0.22em', color: 'var(--ink-35)', textTransform: 'uppercase',
          paddingBottom: 12, marginBottom: 4, borderBottom: '1px solid rgba(255,255,255,0.05)'
        }}>Datos de cuenta</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 14, alignItems: 'start' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <button className="btn" style={{
                  width: '100%', justifyContent: 'center', gap: 7, fontSize: 11.5, padding: '11px',
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (msg.includes('401') || msg.includes('Login failed')) {
        ...
        setError(L('No se pudo conectar con el servidor. Verifica tu conexión.', 'Could not reach the server. Check your connection.'));
      }
    } finally {
      setBusy(false);
    }
...
/* ============================================================
   ACCESO VIEW — Vista de códigos de registro
   ============================================================ */
const AccesoView = () => {
  const L = window.L;
  ...
  const fetchCodes = async () => {
    setBusy(true);
    try {
      const data = await window.ApexAPI.listRegistrationCodes();
      setCodes(data.codes || []);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleGenerate = async () => {
    try {
      await window.ApexAPI.generateRegistrationCode();
      fetchCodes();
    } catch (err) {
      alert(L('No se pudo generar el código.', 'Could not generate code.'));
    }
  };
  ...
  return (
    <div className="s-stage">
      <div className="s-wrap" style={{ maxWidth: 900, padding: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <div>
            <h1 style={{ fontSize: 24, fontWeight: 300, letterSpacing: '-0.01em', marginBottom: 6 }}>
              {L('Códigos de Registro', 'Registration Codes')}
            </h1>
            <p style={{ fontSize: 13, color: 'var(--ink-40)' }}>
              {L('Genera códigos para permitir el registro de nuevos vendedores en tu empresa.', 'Generate codes to allow new sellers to register in your company.')}
            </p>
          </div>
          <button className="btn btn-primary" onClick={handleGenerate} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <SIcon name="sparkle" size={13} /> {L('Generar Código', 'Generate Code')}
          </button>
        </div>
        ...
        <div className="glass" style={{ padding: 0, overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, textAlign: 'left' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.07)', color: 'var(--ink-40)' }}>
                <th style={{ padding: '12px 20px' }}>{L('Código', 'Code')}</th>
                <th style={{ padding: '12px 20px' }}>{L('Creado', 'Created')}</th>
                <th style={{ padding: '12px 20px' }}>{L('Estado', 'Status')}</th>
                <th style={{ padding: '12px 20px' }}>{L('Acciones', 'Actions')}</th>
              </tr>
            </thead>
            <tbody>
              {codes.length === 0 ? (
                <tr>
                  <td colSpan="4" style={{ padding: '30px', textAlign: 'center', color: 'var(--ink-30)', fontStyle: 'italic' }}>
                    {busy ? L('Cargando...', 'Loading...') : L('No hay códigos generados.', 'No codes generated.')}
                  </td>
                </tr>
              ) : (
                codes.map(c => (
                  <tr key={c.code} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    <td style={{ padding: '16px 20px' }}>
                      <span className="mono" style={{ fontSize: 14, fontWeight: 500, color: 'var(--ink-90)' }}>
                        {c.code}
                      </span>
                    </td>
                    <td style={{ padding: '16px 20px', color: 'var(--ink-50)' }}>
                    ...
                    <td style={{ padding: '16px 20px' }}>
                      {c.used_at ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                          <span style={{
                            padding: '3px 8px', borderRadius: 4, fontSize: 11, width: 'fit-content',
                            background: 'rgba(252,165,165,0.1)', border: '1px solid rgba(252,165,165,0.2)', color: '#fca5a5'
                          }}>
                            {L('Usado', 'Used')}
                          </span>
                          {c.used_by_email && (
                            <span style={{ fontSize: 11.5, color: 'var(--ink-40)', wordBreak: 'break-all' }}>
                              {c.used_by_email}
                            </span>
                          )}
                        </div>
                        ...
                        <span style={{
                          ...
                          background: 'rgba(158,245,190,0.1)', border: '1px solid rgba(158,245,190,0.2)', color: '#9ef5be'
                        }}>
                          {L('Activo', 'Active')}
                        </span>
                      )}
                    ...
                    <td style={{ padding: '16px 20px' }}>
                      <button className="btn" onClick={() => handleCopy(c.code)}
                        style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11.5, padding: '5px 10px' }}>
                        <SIcon name="copy" size={12} />
                        {copiedCode === c.code ? L('Copiado', 'Copied') : L('Copiar', 'Copy')}
                      </button>
                    </td>
                  </tr>
                ))
